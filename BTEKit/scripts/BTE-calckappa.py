...
w_file = 'BTE.w_final_full'     # scattering rate (relaxation time)

outfile_kappa_mode = 'BTE.kappa_mode_new'
...
class DATA:
    def __init__(self, pol, omega, tau, vel_x, vel_y, vel_z):
        self.pol = int(pol)
...
kappa_mode = open(outfile_kappa_mode, 'w')      # contribution to kappa of each mode
for j in range(0, 9*nband):
    print >>kappa_mode, "0",
print >>kappa_mode
for i in range(0, NK-1):
    for j in range(0, nband):       # xx
        k=i*nband+j
        de_dT = hBar*data[k].omega/(2*BOLTZ*Teq)
        if (de_dT == 0):
            de_dT = 1
        else:
            de_dT = de_dT/math.sinh(de_dT)
            de_dT = BOLTZ*de_dT*de_dT
        C_mode = de_dT/V
        print >>kappa_mode, "%20.15f" %(C_mode*data[k].vel_x**2*data[k].tau),
    for j in range(0, 3*nband):       # xy xz yx
        print >>kappa_mode, "0",
    for j in range(0, nband):       # yy
        k=i*nband+j
        de_dT = hBar*data[k].omega/(2*BOLTZ*Teq)
        if (de_dT == 0):
            de_dT = 1
            ...
            de_dT = BOLTZ*de_dT*de_dT
        C_mode = de_dT/V
        print >>kappa_mode, "%20.15f" %(C_mode*data[k].vel_y**2*data[k].tau),
    for j in range(0, 3*nband):       # yz zx zy
        print >>kappa_mode, "0",
    for j in range(0, nband): # zz
        k=i*nband+j
        de_dT = hBar*data[k].omega/(2*BOLTZ*Teq)
        if (de_dT == 0):
            de_dT = 1
            ...
            de_dT = BOLTZ*de_dT*de_dT
        C_mode = de_dT/V
        print >>kappa_mode, "%20.15f" %(C_mode*data[k].vel_z**2*data[k].tau),
    print >>kappa_mode
kappa_mode.close()
